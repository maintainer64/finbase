...
describe("Финансовая статистика (view-коллекции)", () => {
    ...
    it("category_sums: возвращает итоги по размеченным категориям", async () => {
        ...
        const namespacedExternalId = `test_stats-cat-tx-${runId}`;
        const txList = await pbRequest(`collections/transactions/records?perPage=200&filter=${encodeURIComponent(`external_id = "${namespacedExternalId}"`)}`, {token});
