...
const FieldInput: Component<FieldInputProps> = (props) => {
    ...
    const render = () => {
        switch (props.field.kind) {
            ...
            case "date":
                return props.field.includeTime ? (
                    <DateTimePicker
                        value={String(props.value ?? "")}
                        onChange={props.onChange}
                    />
            ...
            case "select":
                return (
                    <SearchableSelect
                        value={String(props.value ?? "")}
                        options={(props.field.options ?? []).map((option) => ({value: option, label: option}))}
                        onChange={props.onChange}
            ...
            case "relation":
                return (
                    <SearchableSelect
                        value={String(props.value ?? "")}
                        options={props.options.map((item) => ({value: item.id, ...item}))}
                        onChange={props.onChange}
                        placeholder="Выберите…"
                        emptyLabel={props.field.required ? undefined : "Не выбрано"}
                    />
                );
            case "relation-many":
                return (
            ...
            ? Object.fromEntries(props.spec.fields.map((field) => {
                ...
                if (field.kind === "date") {
                    return [field.name, field.includeTime
                        ? toDateTimeValue(String(raw ?? ""))
                        : toDateValue(String(raw ?? ""))];
                }
            ...
            if (field.kind === "number") {
                ...
                if (field.includeTime) {
                    ...
                    if (Number.isNaN(parsed.getTime())) {
                        toast.error(`Поле «${field.label}» содержит некорректные дату или время`);
                        return;
                    }
                    payload[field.name] = parsed.toISOString();
                } else {
...
const CellValue: Component<CellValueProps> = (props) => {
    ...
    const text = createMemo<string>(() => {
        ...
        if (props.field.kind === "date") return formatDateOnly(String(value));
        return String(value);
    });
                    ...
                    <For each={filterFields()}>
                        {(field) => (
                            <SearchableSelect
                                ...
                                options={filterOptions().get(field.name) ?? []}
                                onChange={(value) => setFieldFilters((current) => ({...current, [field.name]: value}))}
                                placeholder={`Все: ${field.label.toLowerCase()}`}
                                emptyLabel={`Все: ${field.label.toLowerCase()}`}
                            />
                        )}
                    </For>
                    <Show when={dateField()}>
                        <DatePicker class="w-40" value={fromDate()} onChange={setFromDate} placeholder="Дата от"/>
                        <DatePicker class="w-40" value={toDate()} onChange={setToDate} placeholder="Дата до"/>
                    </Show>
                    <Show when={hasAmount()}>
                        <SearchableSelect
                            class="min-w-44"
                            ...
                            onChange={(value) => setAmountKind(value as "" | "income" | "expense")}
                            placeholder="Все движения"
                            emptyLabel="Все движения"
                        />
                    </Show>
