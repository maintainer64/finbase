            ...
            <div class="w-full max-w-xl rounded-3xl border border-white/60 bg-white p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <label>
                        ...
                        <SearchableSelect
                            value={draft().nature}
                            options={[{value: "expense", label: "Расход"}, {value: "income", label: "Поступление"}]}
                            onChange={(value) => change("nature", value as RuleDraft["nature"])}
                        />
                    </label>
                    <label>
                        ...
                        <SearchableSelect
                            ...
                            options={props.categories.map((category) => ({value: category.id, label: category.name, color: category.color, icon: category.lucide_icon}))}
                    ...
                    <label>
                        ...
                        <DatePicker value={draft().effectiveDate} onChange={(value) => change("effectiveDate", value)}/>
                    </label>
            ...
            <div class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-white/60 bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
                ...
                <div class="flex flex-wrap items-center gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-3">
                    <label class="min-w-56 flex-1">
                        ...
                        <SearchableSelect
                            value={category()}
                            options={props.categories.map((item) => ({value: item.id, label: item.name, color: item.color, icon: item.lucide_icon}))}
                                    ...
                                    <label class="flex cursor-pointer items-center gap-3 px-5 py-3 transition hover:bg-blue-50/40">
                                        ...
                                        <span class="min-w-0 flex-1">
                                            <span class="flex min-w-0 items-center gap-2">
                                                ...
                                                <span class="hidden shrink-0 text-xs text-slate-400 sm:inline">{formatDate(transaction.date)}</span>
                                            </span>
                                            <span class="block truncate text-xs text-slate-400">{props.accountLabel(transaction.account)}<span class="sm:hidden"> · {formatDate(transaction.date)}</span></span>
                                        </span>
                                ...
                                <div class="divide-y divide-slate-100">
                                    <For each={items()}>{(transaction) => (
                                        <label class="flex cursor-pointer items-center gap-3 px-5 py-3 transition hover:bg-violet-50/40">
                                            ...
                                            <span class="min-w-0 flex-1">
                                                <span class="flex min-w-0 items-center gap-2"><span class="truncate text-sm font-medium text-slate-800">{transaction.note || "Без описания"}</span><span class="hidden shrink-0 text-xs text-slate-400 sm:inline">{formatDate(transaction.date)}</span></span>
                                                <span class="block truncate text-xs text-slate-400">{props.accountLabel(transaction.account)}<span class="sm:hidden"> · {formatDate(transaction.date)}</span></span>
                                            </span>
    ...
    const transactionCard = (transactionId: string, direction: "in" | "out") => {
        ...
        return (
            <div class="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                ...
                <div class="mt-1 text-xs text-slate-400">{formatDate(transaction?.date)}</div>
            </div>
        );
    };
                                ...
                                <For each={groups()}>{(group) => (
                                    <div class="grid items-center gap-3 px-5 py-4 md:grid-cols-[minmax(0,1.4fr)_110px_150px_minmax(180px,.8fr)_auto]">
                                        ...
                                        <SearchableSelect
                                            ...
                                            options={categories().map((category) => ({value: category.id, label: category.name, color: category.color, icon: category.lucide_icon}))}
                                            onChange={(value) => setGroupCategories((current) => ({...current, [group.id]: value}))}
                                            placeholder="Выберите категорию…"
                                        />
                                        <div class="flex flex-wrap justify-end gap-2">
