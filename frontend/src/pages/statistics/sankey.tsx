            ...
            tooltip: {
                ...
                formatter: (params: {dataType?: string; data?: SankeyNode | SankeyLink; value?: number}) => {
                    if (params.dataType === "edge") {
                        ...
                        return `<b>${escapeChartHtml(source)} → ${escapeChartHtml(target)}</b><br/>${formatMoney(link.value, props.currency)}`;
                    }
                    const node = params.data as SankeyNode;
                    return `<b>${escapeChartHtml(node.displayName)}</b><br/>${formatMoney(Number(params.value ?? 0), props.currency)}`;
                },
            },
            series: [{
                type: "sankey",
