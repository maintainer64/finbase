...
import {CircleDollarSign, LoaderCircle} from "lucide-solid";
...
const accountCurrency = (account: AccountRecord): string => (account.currency || "RUB").trim().toUpperCase();
...
export const StatisticsPage: Component = () => {
    ...
    const query = createMemo<QueryState>(() => {
        ...
        return {
            currency: (p.get("currency") || "RUB").toUpperCase(),
    ...
    const selectedCurrency = createMemo(() => query().currency);
    const currencyOptions = createMemo(() => [...new Set([
        "RUB",
        selectedCurrency(),
        ...accounts().map(accountCurrency),
    ])]
        .filter(Boolean)
        .sort((left, right) => left === "RUB" ? -1 : right === "RUB" ? 1 : left.localeCompare(right))
        .map(currency => ({value: currency, label: currency})));

    const accountsByCurrency = createMemo(() => accounts()
        .filter(account => accountCurrency(account) === selectedCurrency()));

    // Счета выбранной валюты, прошедшие фильтры типа, владельца и банка.
    const accountsByGroup = createMemo(() => {
        ...
        return accountsByCurrency().filter(a =>
    ...
    const providerOptions = createMemo(() => [...new Set(accountsByCurrency()
    ...
    return (
        ...
        <div class="page-shell">
            <div class="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-4 shadow-sm">
                <div class="flex min-w-0 items-center gap-3">
                    <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white shadow-sm">
                        <CircleDollarSign size={21}/>
                    </span>
                    <div>
                        <div class="text-xs font-semibold uppercase tracking-[.14em] text-blue-500">Валюта отчёта</div>
                        <div class="mt-0.5 text-sm text-slate-500">Счета и графики рассчитываются только в одной валюте</div>
                    </div>
                </div>
                <SearchableSelect
                    class="w-44"
                    ...
                    onChange={(currency) => updateQuery({currency, accounts: []})}
                    searchPlaceholder="Найти валюту…"
                    clearable={false}
                />
            </div>
            ...
            <div class="flex flex-col xl:flex-row gap-5 items-start">
                ...
                <div class="flex-1 flex flex-col gap-4 min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 text-xs text-gray-500">
                            <div class="flex items-center gap-1">
                                ...
                                <DatePicker
                                    class="w-36"
                                    value={query().from}
                                    onChange={(value) => updateQuery({period: "", from: value})}
                                />
                            </div>
                            <div class="flex items-center gap-1">
                                <span>До</span>
                                <DatePicker
                                    class="w-36"
                                    value={query().to}
                                    onChange={(value) => updateQuery({period: "", to: value})}
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <Show when={!accountsLoading()}>
                        <div class="summary-grid">
                            <div class="summary-card summary-card--balance">
                                ...
                                <strong class="summary-card__value">{fmtMoney(totalBalance(), selectedCurrency())}</strong>
                                <span class="summary-card__hint">На {effectiveAccounts().length} счетах · {selectedCurrency()}</span>
                            </div>
                            <div class="summary-card">
                                ...
                                <strong class="summary-card__value text-emerald-700">{loading() ? "—" : fmtMoney(periodIncome(), selectedCurrency())}</strong>
                            ...
                            <div class="summary-card">
                                ...
                                <strong class="summary-card__value text-rose-700">{loading() ? "—" : fmtMoney(periodExpense(), selectedCurrency())}</strong>
                                <span class="summary-card__hint">{loading() ? "Загружаем агрегаты…" : "За выбранный период"}</span>
                            </div>
                            <div class="summary-card">
                                ...
                                <strong class={`summary-card__value ${periodResult() >= 0 ? "text-emerald-700" : "text-rose-700"}`}>
                                    {loading() ? "—" : fmtMoney(periodResult(), selectedCurrency())}
                                </strong>
                    ...
                    <Show when={!loading() && accounts().length > 0 && accountsByCurrency().length === 0 && !error()}>
                        <div class="status-message">Нет счетов в валюте {selectedCurrency()}. Выберите другую валюту или добавьте подходящий счёт.</div>
                    </Show>

                    <Show when={!loading() && accountsByCurrency().length > 0 && flows().length === 0 && !error()}>
            ...
            <Show when={overallPoints().length > 0}>
                <Space>
                    <div class="flex items-baseline justify-between mb-1">
                        ...
                        <span class="text-xl font-semibold tabular-nums">{fmtMoney(totalBalance(), selectedCurrency())}</span>
                    </div>
                    <div class="h-64">
                        <BalanceChart series={overallChartSeries()} currency={selectedCurrency()} area/>
                    </div>
                </Space>
            </Show>
            ...
            <Show when={seriesByAccount().size > 0}>
                <Space>
                    ...
                    <div class="h-72">
                        <BalanceChart series={accountChartSeries()} currency={selectedCurrency()}/>
                    </div>
                </Space>
            </Show>
                                ...
                                <For each={incomeDonutItems()}>
                                    {(item) => (
                                        <button class="flex items-center gap-2 rounded px-1.5 py-1 text-left text-xs hover:bg-emerald-50" onClick={() => { if (categoryTree().has(item.cat.id)) setDrillLevel(item.cat.id); }}>
                                            ...
                                            <span class="shrink-0 tabular-nums text-emerald-700">{fmtMoney(item.income, selectedCurrency())}</span>
                                        </button>
                                    )}
                                </For>
                            </div>
                                ...
                                <For each={expenseDonutItems()}>
                                    {(item) => (
                                        <button class="flex items-center gap-2 rounded px-1.5 py-1 text-left text-xs hover:bg-rose-50" onClick={() => { if (categoryTree().has(item.cat.id)) setDrillLevel(item.cat.id); }}>
                                            ...
                                            <span class="min-w-0 flex-1 truncate text-gray-700">{item.cat.name}</span>
                                            <Show when={categoryTree().has(item.cat.id)}><span class="text-gray-300">›</span></Show>
                                            <span class="shrink-0 tabular-nums text-rose-700">{fmtMoney(item.expense, selectedCurrency())}</span>
                                        </button>
                                    )}
                                </For>
                            </div>
            ...
            <Show when={expenseTotal() > 0}>
                <Space>
                    ...
                    <Sankey accounts={accountsByCurrency()} categories={categories()} splits={filteredSplits()} currency={selectedCurrency()}/>
                </Space>
            </Show>
