...
func detectTransfer(app core.App, transaction *core.Record, window time.Duration) error {
	...
	if amount == 0 || date.IsZero() || transaction.GetString("category") != "" || transactionHasActiveTransfer(app, transaction.Id) {
		return nil
	}
	candidates, err := app.FindRecordsByFilter(
		"transactions",
		"id != {:id} && category = '' && account != {:account} && currency = {:currency} && amount = {:opposite} && date >= {:from} && date <= {:to}",
		"date",
		0,
		0,
		dbx.Params{
